...
void OpenSMOKE_SolidExperiment::ReadFromFile(const string filename)
{
	...
	fInput >> dummy;
	if (dummy!="PHI")	ErrorMessage("Expected: PHI - Found: " + dummy);
	fInput >> PHI;
	...
	{
		kind_of_experimental_data = Char_CONVERSION_CHAR;
		for(;;)
		{
			fInput >> dummy;
			break;
	...
	else if (y_type == "D_CONVERSION_CHAR")
	{
		kind_of_experimental_data = Char_D_CONVERSION_CHAR;
		for(;;)
		{
			fInput >> dummy;
			break;
	...
	else if (y_type == "MASS_RATIO")
	{
		kind_of_experimental_data = Char_MASS_RATIO;
		for(;;)
		{
			fInput >> dummy;
	...
	{
		kind_of_experimental_data = Char_D_MASS_RATIO;
		for(;;)
		{
			fInput >> dummy;
			if (dummy == "UNITS") break;
	...
	else	ErrorMessage("Expected: CONVERSION_CHAR || D_CONVERSION_CHAR || MASS_RATIO || D_MASS_RATIO - Found: " + y_type);
	...
	for(;;)
	{
		...
		if (y_type == "D_CONVERSION_CHAR")	y.Append(OpenSMOKE_Conversions::conversion_frequency(atof(dummy.c_str()), y_units));
	}
	fInput.close();
