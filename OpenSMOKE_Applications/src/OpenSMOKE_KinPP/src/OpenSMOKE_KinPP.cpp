...
CSimpleOpt::SOption parser_options[] =
{
	...
	{ OPT_ARG,  _T("--clustering"),      SO_REQ_SEP }, // "--clustering		ARG"
	...
	{ OPT_ARG,  _T("-backup"),			 SO_NONE    }, // "-backup			ARG"
	{ OPT_ARG,  _T("-only-ode"),		 SO_NONE    }, // "-only-ode		ARG"
	{ OPT_ARG,  _T("-only-clustering"),  SO_NONE    }, // "-only-ode		ARG"
...
void ParserClass::ShowUsage()
{
	...
	cout << "   -only-clustering        only clustering"									<< endl;
...
{	
    ...
    if (parser.parse("--kinetics", argument))
    {
        ...
        mix.SetupBinary(kineticSchemePath);
	}
	...
	else iSequence = false;

	// 3. Jacobian
 //   #if SYMBOLIC_KINETICS == 1
	if (parser.parse("-jacobian"))
	{
		iAnalyticalJacobian = 1;
		string kineticSchemeName = GiveMeFileNameFromFullPath(kineticSchemePath);
	/*	if		(kineticSchemeName == "Polimi_C1C3HTNOX_0810")	analyticalJacobian = POLIMI_C1C3HTNOX_0810;
		else if (kineticSchemeName == "Polimi_C1C3HTNOX_AVIO")	analyticalJacobian = POLIMI_C1C3HTNOX_AVIO;
		...
		*/
		if (kineticSchemeName == "GRI12")	analyticalJacobian = GRI12;
		else ErrorMessage("Symbolic Jacobian not available for this kinetic scheme");
	}
	else
	{
		iAnalyticalJacobian = 0;
		analyticalJacobian	= NOJACOBIAN;
	}
/*	#else
	{
		iAnalyticalJacobian = 0;
	...
	*/
	...
	{
		// Clustering
		if (parser.parse("--clustering", argument))
			cicloCluster = atof(argument.c_str());
		else ErrorMessage("The --clustering option is compulsory");

		// Relaxation
		if (parser.parse("--relaxation", argument))
			relaxation = atoi(argument.c_str());
		else relaxation = 0;
	}
	...
	if (iSequence == true)
	{
		// Clustering
		if (parser.parse("--clustering", argument))
			ErrorMessage("The --clustering option cannot be used in --sequence mode");

		// Relaxation
		if (parser.parse("--relaxation", argument))
			ErrorMessage("The --relaxation option cannot be used in --sequence mode");
	}
	...
	if (iSequence == true)
	{
		...
		for(;;)
		{
			...
			if	(dummy_string == "#RUN")
			{
				fSequence >> dummy_double;	iClusterList.Append(dummy_double);
				fSequence >> dummy_int;		iCycleList.Append(dummy_int);
			}
