...
{ 
    cout << endl;
    cout << "Executable:  OpenSMOKE_Batch"		<< endl;
...
int main(int argc, char* argv[])
{
    ...
    if (parser.parse("--output", argument))
		batch.SetOutputFolder(argument);


    // 4. Batch Setup
    batch.AssignKineticScheme(mix);
    batch.AssignGlobalKineticScheme(global);
	batch.AssignSoot2EModel(soot2EModel);
    batch.AssignInletFlows(inlet);
    if (parser.parse("--input", argument))
            batch.DefineFromFile(argument);
    else    batch.DefineFromFile("Batch.inp");
    ...
    // 7. Video Solution
    batch.VideoFinalResult();

	// 8. Outlet Stream
	batch.OutletStream(outlet);
	outlet.VideoSummary();

	// 9. Mass and Energy balances
	batch.SummaryOnFile();
	batch.MassAnalysis(outlet);
	batch.EnergyAnalysis(outlet);

    OpenSMOKE_logo("OpenSMOKE_Batch", "0.3", "January 2014");
...
{
    cout << "Usage: OpenSMOKE_Batch [-option] [--option STRING] [-?] [--help]"						<< endl;
