...
void OpenSMOKE_CharKineticScheme::ParsingReactions(const int iReaction, const int iLine, vector<string> instructions)
{
	...
	// Reactants
	{
		...
		for(i=1;i<=index-1;i++)
		{
			...
			cout << iReaction << " " << number << " " << name << endl;
		}
...
void OpenSMOKE_CharKineticScheme::MemoryAllocation()
{
	...
	ChangeDimensions(			number_of_reactions,	&nuChar);
	...
	for(i=1;i<=number_of_reactions;i++)
	{
		...
		for(int j=1;j<=nAdsorbed;j++)
			sum += nuAdsorbed[j][i]*GetEta(speciesAdsorbed[j]);
		for(int j=1;j<=nGas;j++)
			sum += nuGas[j][i]*GetEta(speciesGas[j]);
		nuChar[i] = nuCarbon[1][i] + sum;				// TODO
	}
...
void OpenSMOKE_CharKineticScheme::ReadDatabase(const string file_name)
{
	...
	database_species.push_back("eta species");
...
void OpenSMOKE_CharKineticScheme::Summary(ofstream &fOutput)
{
	...
	fOutput << "--------------------------------------------------------------------------------------" << endl;
	fOutput << "                         Char Stoichiometric coefficients                             " << endl;
	fOutput << "--------------------------------------------------------------------------------------" << endl;
	fOutput << setw(14) << "";
	for(i=1;i<=number_of_reactions;i++)
		fOutput << setw(8) << left << i;
	...
	{
		fOutput << setw(14) << speciesCarbon[i];	// TODO
		for(int j=1;j<=number_of_reactions;j++)
			fOutput << setw(8) << left << fixed << setprecision(3) << nuChar[j];
		fOutput << endl;
	}
	fOutput << endl << endl;
...
{
	for(int j=1;j<=number_of_reactions;j++)
		kappa[j] = A[j]*exp(-E[j]/Constants::R_J_kmol/T);		// [kmol, m, s]
}

void OpenSMOKE_CharKineticScheme::UpdateKineticConstants(BzzVector &Ccarbon, BzzVector &Cbulk, BzzVector &Cgas,
														 BzzVector &Cadsorbed)
{
	int i,j;
	...
	for(j=1;j<=number_of_reactions;j++)	// Gas
		for(i=1;i<=nGas;i++)
			rr[j] *= pow(Cgas[i], lambdaGas[i][j]);					// [kmol/m2/s]
