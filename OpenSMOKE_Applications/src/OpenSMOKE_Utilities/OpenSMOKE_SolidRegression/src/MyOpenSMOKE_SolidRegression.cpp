...
void MyOpenSMOKE_SolidRegression::SetParameters(const vector<string> names)
{
		...
		ErrorMessage("Available options: ALL || A || BETA || E || A_E || BETA_E || A_BETA || USERDEFINED");
	...
	{
		ChangeDimensions(int(names.size()-1)/3, 2, &bUserDefined);
		int j=1;	int i=1;
		for(;;)
		{
			if (names[i] != "REACTION")	ErrorMessage("Expected REACTION key word - Found: " + names[i]);
			...
			else ErrorMessage("Expected A || BETA || E - Found: " + names[i+2]);
...
void MyOpenSMOKE_SolidRegression::Run_Char()
{
	...
	kinetics->UpdateOptimizerParameters(iModel, parameters, bFirstGuess, bUserDefined);
	...
	kinetics->UpdateKineticParameters(iModel, parameters, b, bUserDefined);
	...
	for(i=1;i<=nCases;i++)
	{
		...
		// Original data
		{
			kinetics->UpdateKineticParameters(iModel, parameters, bFirstGuess, bUserDefined);
...
void MyOpenSMOKE_SolidRegression::GetModel_01(BzzVector &y, double t, BzzVector &f)
{
	...
	if (iSurfaceEquation == true)
	{
		...
		dxchar_over_dt = experiments_char[ptIndex].Sg0*Sg * MWchar * kinetics->Rchar;				// [1/s]

		// Equation 2: Surface
		dSg_over_dt = ( experiments_char[ptIndex].Psi*(1.-xchar)/2./Sg - Sg/(1.-xchar+1.e-12) ) * dxchar_over_dt;		// [m2/kg/s]
...
void MyOpenSMOKE_SolidRegression::GetModel_02(BzzVector &y, double t, BzzVector &f)
{
	...
	for(i=1;i<=indices_solid.Size();i++)
		mass_total_solid += mass[indices_solid[i]];
...
void MyOpenSMOKE_SolidRegression::WriteOptimizedParametersBio(const string file_name)
{
	...
	for(j=1;j<=mix_chemkin->NumberOfReactions();j++)
	{
		...
		fOutput << "R" << setw(6) << left << j;
		fOutput << " / ";
		fOutput << setw(8) << left << fixed << mix_chemkin->kinetics.GiveMe_A(j)/pow(10., double(exponent));
		fOutput << " / ";
		fOutput << setw(4) << left << fixed << exponent;
		fOutput << " / ";
		fOutput << setw(8) << left << fixed << mix_chemkin->kinetics.GiveMe_Beta(j);
		fOutput <<" / ";
		fOutput << setw(8) << left << fixed << mix_chemkin->kinetics.GiveMe_E(j)/Constants::Conversion_cal_J;
		fOutput << " / ";
		fOutput << endl;
	}
