...
void OpenSMOKE_KPP_BlockMatrixNetwork::ErrorMessage(const std::string message_)
{
    std::cout << std::endl;
    std::cout << "Class: OpenSMOKE_KPP_BlockMatrixNetwork"	<< std::endl;
...
void OpenSMOKE_KPP_BlockMatrixNetwork::WarningMessage(const std::string message_)
{
    std::cout << std::endl;
    std::cout << "Class:	  OpenSMOKE_KPP_BlockMatrixNetwork"	<< std::endl;
...
int OpenSMOKE_KPP_BlockMatrixNetwork::GaussSiedel(BzzVector& solution)
{
	...
	std::cout << std::setw(12) << "Iteration" << std::setw(16) << "NormInf" << std::setw(16) << "Norm1" << std::setw(16) << "Norm1(mean)" << std::endl;
	...
	for(iteration=1;iteration<=maxIterations_;iteration++)
	{
		...
		std::cout << std::setw(12) << iteration << std::setw(16) << maxDxGlobal_ << std::setw(16) << maxXGlobal_ << std::setw(16) << maxXGlobal_/double(NE_) << std::endl;

		if( maxDxGlobal_ < relTolerance_*maxXGlobal_ + double(NE_)*absTolerance_)
		{
			std::cout << "Solution succesfully reached in Block-Gauss-Siedel mode..." << std::endl;
			break;
		}
		...
		else
		{
			...
			if( (countFailure_>100) || (maxDxGlobal_>10.*maxXGlobal_) || (maxDxGlobal_ > 1.e10) )
			{
				if (countFailure_>100)
					std::cout << "Failure of Block-Gauss-Siedel mode: max number of failures..." << std::endl;
				if (maxDxGlobal_>10.*maxXGlobal_)
					std::cout << "Failure of Block-Gauss-Siedel mode: max relative dx..." << std::endl;
				if (maxDxGlobal_ > 1.e10)
					std::cout << "Failure of Block-Gauss-Siedel mode: max absolute dx..." << std::endl;
				return -1;
			}
		}
	}
	...
	std::cout << "Block Gauss-Siedel CPU Time: " << timeEnd-timeStart << " (" << (timeEnd-timeStart)/double(iteration) << ")" << std::endl;
	...
	{
		std::cout << "Maximum number of iterations in Block-Gauss-Siedel mode..." << std::endl;
		return 1;
	}
