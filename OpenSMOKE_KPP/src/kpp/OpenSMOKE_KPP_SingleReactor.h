...
public:
	...
	void Setup(const int i_local, const int i_global, OpenSMOKE_ReactingGas* mixture, BzzVectorInt& indexReduced);
	void SetDataManager(OpenSMOKE_KPP_DataManager* data);
	void ResetStatus();
	bool ReadMassFractions(std::ifstream &fInput);
	bool ReadReactorProperties(std::ifstream &fInput);
	bool ReadTopology(std::ifstream &fInput);
	...
	void GetJacobian(BzzVector &omega,BzzMatrix &JJ);
        void EnergyOutflow(OpenSMOKE_ReactingGas* mixture);
        void EnergyInflow(OpenSMOKE_ReactingGas* mixture, OpenSMOKE_KPP_ReactorNetwork& network);
	...
	void Residuals(OpenSMOKE_KPP_ReactorNetwork& network);
	...
	inline double fOutTot() const { return fOut_; }
        inline double H_in()    const { return H_in_;}
        inline double H_out()    const { return H_out_;}
        inline double H_mix()    const { return H_mix_;}
        inline double cp_mix()  const { return cp_mix_;}
...
//	inline const BzzVectorInt& globalIndicesSparsityColumns() const { return globalIndicesSparsityColumns_; }
	...
	inline const BzzVector&    norms()                const { return norms_; }
	...
	inline void setOmega(BzzVector &massFractions) { omega_ = massFractions; }
	       ...
	       void setOmegaFromNetwork(const int k, const BzzVector &massFractions, OpenSMOKE_KPP_ReactorNetwork& network);
	       void setOmegaFromNetwork(const int k, const Vec &massFractions, OpenSMOKE_KPP_ReactorNetwork& network);  
	...
	double M_aux;
        double cp_mix_;
	BzzVectorInt iConvDiff_aux;
	BzzVector mConvDiff_aux;
...
//	BzzOdeStiffObject o;
	...
	int count_;
	PetscScalar *omega_value;

	unsigned int index_;
	unsigned int index_local_;
	int numberOfSpecies;
