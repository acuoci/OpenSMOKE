...
void OpenSMOKE_KPP_SingleReactor::ErrorMessage(const std::string message_)
{
    std::cout << std::endl;
    std::cout << "Class: OpenSMOKE_KPP_SingleReactor"	<< std::endl;
...
void OpenSMOKE_KPP_SingleReactor::WarningMessage(const std::string message_)
{
    std::cout << std::endl;
    std::cout << "Class:	  OpenSMOKE_KPP_SingleReactor"	<< std::endl;
    std::cout << "Index:   " << index_						<< std::endl;
...
void OpenSMOKE_KPP_SingleReactor::Setup(const int i_local, const int i_global, OpenSMOKE_ReactingGas* mixture, BzzVectorInt& indexReduced)
{
	index_ = i_global;
	index_local_ = i_local;
...
bool OpenSMOKE_KPP_SingleReactor::ReadMassFractions(std::ifstream &fInput)
{
...
bool OpenSMOKE_KPP_SingleReactor::ReadReactorProperties(std::ifstream &fInput)
{
	count_ = 0;
	unsigned int dummy_int;

	fInput >> dummy_int; count_++;

	if (dummy_int != index_)
		ErrorMessage("Indices do not fit!");

	fInput >> temperature_; count_++;
	fInput >> pressure_; count_++;
	fInput >> volume_; count_++;
	fInput >> variance_; count_++;
...
bool OpenSMOKE_KPP_SingleReactor::ReadTopology(std::ifstream &fInput)
{
	// External feeds
	{
		unsigned int numberInlets;
		fInput >> numberInlets; count_++;
		...
		for (int k=1; k<=int(numberInlets); k++)
		{
			...
			double massFlow;
			fInput >> massFlow; count_++;
			fInput >> numberSpecies; count_++;
			for (unsigned int j=1; j<=numberSpecies; j++)
			{
				unsigned int indexSpecies;
				double omegaSpecies;
				fInput >> indexSpecies >> omegaSpecies; count_ += 2;
	...
	// Outputs
	{
		...
		{
			fInput >> out_[k] >> cOut_[k] >> diffusion_[k]; count_ += 3;
...
{
	// Get dR_over_domega
	kinetics->GetFormationRatesDerivatives(index_omp_, omega, temperature_, pressure_, JJ);
...
void OpenSMOKE_KPP_SingleReactor::ODESystemContinousReactor(BzzVector &omega, double t, BzzVector &domega)
{
	...
	{
		// Formation rates
		kinetics->UpdateProperties(index_omp_, omega, temperature_, pressure_, R_);
...
{
	kinetics->UpdateProperties(index_omp_, omega_, temperature_, pressure_, R_);
	Product(volume_, R_, &RV_);
	residuals = RV_; 
}

void OpenSMOKE_KPP_SingleReactor::Residuals(OpenSMOKE_KPP_ReactorNetwork& network)
{
	// Reactions
	...
	// Inflow
	for(int i=1;i<=iConvectionDiffusion_.Size();i++)
	{
	    for(int j = 1; j <= numberOfSpecies; j++)
	        residuals_[j] += mConvectionDiffusion_[i] * network.OmegaGlob()[iConvectionDiffusion()[i]][j];
	}

	// Outflow
	residuals_ -= M_*omega_;

    	norms_[1] = residuals_.MaxAbs();
    	norms_[2] = residuals_.GetSumAbsElements();
    	norms_[3] = BzzPow2(residuals_.Norm2());
...
{
	ChangeDimensions(iConvDiff_aux.Size(), &iConvectionDiffusion_);
	ChangeDimensions(mConvDiff_aux.Size(), &mConvectionDiffusion_);

	for(int j = 1; j <= iConvDiff_aux.Size(); j++)
    	{
            iConvectionDiffusion_[j] = iConvDiff_aux[j];
            mConvectionDiffusion_[j] = mConvDiff_aux[j];
        }
...
void OpenSMOKE_KPP_SingleReactor::setOmegaFromNetwork(const int k, const BzzVector &massFractions, OpenSMOKE_KPP_ReactorNetwork& network)
{
	int position = (k-1)*numberOfSpecies;
	for(int i=1;i<=numberOfSpecies;i++)
		omega_[i] = massFractions[position+i];
...
void OpenSMOKE_KPP_SingleReactor::setOmegaFromNetwork(const int k, const Vec &massFractions, OpenSMOKE_KPP_ReactorNetwork& network)
{
	int position = (k-1)*numberOfSpecies;
	VecGetArray(massFractions, &omega_value);
	for(int i = 1; i<=numberOfSpecies; i++)
	    omega_[i] = omega_value[position + i];

	VecRestoreArray(massFractions, &omega_value);
}
...
void OpenSMOKE_KPP_SingleReactor::CheckMassFractions(BzzVector& omega, const double epsilon)
{
	...
	if(sum > (1.+1.e-2) || sum < (1.-1.e-2))
	{
		std::cout << "Error in sum of mass fractions: " << sum-1. << " (+-" << epsilon << ")" << std::endl;
...
double OpenSMOKE_KPP_SingleReactor::NewtonStepReductionFactor(BzzVector& omega, BzzVector& direction)
{
	...
	for(int i=1;i<=numberOfSpecies;i++)
	{
		if(direction[i] < 0.)  
		{
			...
			{
				hMin = std::min(hMin, -omega[i]/direction[i]);
			}
		}
	}
...
void OpenSMOKE_KPP_SingleReactor::EnergyOutflow(OpenSMOKE_ReactingGas* mix)
{
    H_mix_ = 0;
    H_out_ = 0;
    BzzVector cell_h(numberOfSpecies);

    //Enthalpy
    mix->SpeciesEnthalpy(temperature_);
    Product((Constants::R_J_kmol * temperature_), mix->h, &cell_h); // [J/kmol]
    ElementByElementProduct(cell_h, mix->uM, &cell_h); // [J/kg]

    //Mixture enthalpy and enthalpy flux
    H_mix_ = Dot(cell_h, omega_);

    //Evaluating heat power exiting the reactor
    H_out_ = M() * H_mix_; // [J/s]
    
}

void OpenSMOKE_KPP_SingleReactor::EnergyInflow(OpenSMOKE_ReactingGas* mix, OpenSMOKE_KPP_ReactorNetwork& network)
{
    H_in_ = 0;
    BzzVector H_feed(numberOfSpecies);
    
    //Adding external contributes, if present
    if (tagExternalFeed_ == true)
    {
        mix->SpeciesEnthalpy(temperature_);
        Product((Constants::R_J_kmol * temperature_), mix->h, &H_feed); // [J/kmol]
        ElementByElementProduct(H_feed, mix->uM, &H_feed); // [J/kg]

        H_in_ = Dot(H_feed, fIn_); // [J/s]
    }
    
    for (int j = 1; j <= iConvectionDiffusion_.Size(); j++)
    {
        H_in_ += mConvectionDiffusion_[j] * network.Hmix()[iConvectionDiffusion_[j]];
    }    
...
/*    if (index_ == 210)
    {
        for(int i = 1; i <= numberOfSpecies; i++)
        {
            std::cout << fIn_[i] << "    " << H_feed[i] << std::endl;
        }

        std::cout << H_out_ << "   " << M() << "    "  << mConvectionDiffusion_.GetSumElements() + fIn_.GetSumElements() << std::endl;
        getchar();
    }*/
    
    mix->SpeciesCp(temperature_);
    cp_mix_ = mix->MixCp_FromMassFractions(omega_);
}