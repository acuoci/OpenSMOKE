...
OpenSMOKE_LiquidVaporizationHeat_Dictionary::OpenSMOKE_LiquidVaporizationHeat_Dictionary()
{
...
void OpenSMOKE_LiquidVaporizationHeat_Dictionary::ReadFromFile(const string file_name)
{
	...
	PrepareLiquidPropertiesDictionary("Liquid Vaporization Heat Database", file_name, lines, indexLines);
	...
	ChangeDimensions(total_number_of_species, &Hvmin);
	ChangeDimensions(total_number_of_species, &Hvmax);
...
void OpenSMOKE_LiquidVaporizationHeat_Dictionary::ProcessData(const string line, const int iLine, const int index)
{
	...
	if (instructions.size()-1 != 15)
		ErrorMessage("Syntax error in vaporization heat coefficient definition", iLine);
	...
	Hvmin[index]			= atof(instructions[12].c_str());
	Tmax[index]				= atof(instructions[13].c_str());
	Hvmax[index]			= atof(instructions[14].c_str());

	int dummy_int			= atoi(instructions[15].c_str());
	
	if (dummy_int == 1)		equation[index]	= liquid_vaporizationheat_equation::EQ1;
...
void OpenSMOKE_LiquidVaporizationHeat_Dictionary::WriteToFile(const string file_name)
{
	...
	fOutput << setw(16) << left << "Hvmin";
	fOutput << setw(16) << left << "Tmax";
	fOutput << setw(16) << left << "Hvmax";
	...
	for(int i=1;i<=CAS.Size();i++)
	{
		...
		fOutput << setw(16) << left << scientific << Hvmin[i];
		fOutput << setw(16) << left << scientific << Tmax[i];
		fOutput << setw(16) << left << scientific << Hvmax[i];

		if (equation[i] == liquid_vaporizationheat_equation::EQ1)	fOutput << setw(4) << left << fixed << 1;
...
void OpenSMOKE_LiquidVaporizationHeat_Dictionary::SaveToFile(BzzSave &fSave)
{
	...
	for(int i=1;i<=C1.Size();i++)
		if (equation[i] == liquid_vaporizationheat_equation::EQ1)	equation_int[i] = 1;
