...
OpenSMOKE_LiquidVaporPressure_Dictionary::OpenSMOKE_LiquidVaporPressure_Dictionary()
{
...
void OpenSMOKE_LiquidVaporPressure_Dictionary::ReadFromFile(const std::string file_name)
{
	...
	PrepareLiquidPropertiesDictionary("Liquid Vapor Pressure Database", file_name, lines, indexLines);
	...
	ChangeDimensions(total_number_of_species, &Pvmin);
	ChangeDimensions(total_number_of_species, &Pvmax);
...
void OpenSMOKE_LiquidVaporPressure_Dictionary::ProcessData(const std::string line, const int iLine, const int index)
{
	...
	if (instructions.size()-1 != 15)
		ErrorMessage("Syntax error in vapor pressure coefficient definition", iLine);
	...
	C1[index]				= atof(instructions[6].c_str());
	C2[index]				= atof(instructions[7].c_str());
	C3[index]				= atof(instructions[8].c_str());
	C4[index]				= atof(instructions[9].c_str());
	C5[index]				= atof(instructions[10].c_str());
	Tmin[index]				= atof(instructions[11].c_str());
	Pvmin[index]			= atof(instructions[12].c_str());
	Tmax[index]				= atof(instructions[13].c_str());
	Pvmax[index]			= atof(instructions[14].c_str());

	int dummy_int			= atoi(instructions[15].c_str());
	
	if (dummy_int == 1)	equation[index]	= liquid_vaporpressure_equation::EQ1;
...
void OpenSMOKE_LiquidVaporPressure_Dictionary::WriteToFile(const std::string file_name)
{
	...
	fOutput << setw(16) << left << "Pvmin";
	fOutput << setw(16) << left << "Tmax";
	fOutput << setw(16) << left << "Pvmax";
	...
	for(int i=1;i<=CAS.Size();i++)
	{
		...
		fOutput << setw(16) << left << scientific << Pvmin[i];
		fOutput << setw(16) << left << scientific << Tmax[i];
		fOutput << setw(16) << left << scientific << Pvmax[i];

		if (equation[i] == liquid_vaporpressure_equation::EQ1)	fOutput << setw(4) << left << fixed << 1;
...
void OpenSMOKE_LiquidVaporPressure_Dictionary::SaveToFile(BzzSave &fSave)
{
	...
	for(int i=1;i<=C1.Size();i++)
		if (equation[i] == liquid_vaporpressure_equation::EQ1)	equation_int[i] = 1;
