...
void OpenSMOKE_PostProcessor_ICEM_MultiZone::ErrorMessage(const std::string message)
{
    cout << endl;
    cout << "Class:  OpenSMOKE_PostProcessor_ICEM_MultiZone"		<< endl;
...
void OpenSMOKE_PostProcessor_ICEM_MultiZone::WarningMessage(const std::string message)
{
    cout << endl;
    cout << "Class:  OpenSMOKE_PostProcessor_ICEM_MultiZone"	<< endl;
    cout << "Object: "		<< name_object				<< endl;
...
OpenSMOKE_PostProcessor_ICEM_MultiZone::OpenSMOKE_PostProcessor_ICEM_MultiZone(OpenSMOKE_PostProcessor *post_processor_) 
: OpenSMOKE_PostProcessor_General(post_processor_)
{
}

void OpenSMOKE_PostProcessor_ICEM_MultiZone::ReadFromBinaryFile(BzzLoad &fLoad)
{
	...
	// Grid
	cout << "Reading TAU..." << endl;
	CheckInBinaryFile(fLoad, "TAU");
	fLoad >> x;
	
	CheckInBinaryFile(fLoad, "TETA");
	fLoad >> csi;
	...
	cout << "ICEM-MultiZone succesfully loaded..." << endl;

	Prepare();
}

void OpenSMOKE_PostProcessor_ICEM_MultiZone::Prepare()
{
	list_of_x_available.resize(0);
	list_of_x_available.push_back("time");
	list_of_x_available.push_back("angle");
	...
	list_of_x_labels.push_back("angle [deg]");
...
void OpenSMOKE_PostProcessor_ICEM_MultiZone::ExportAvailableXAxis(vector<string> &x_available)
{
	x_available = list_of_x_available;
}	

void OpenSMOKE_PostProcessor_ICEM_MultiZone::ExportAvailableYAxis(vector<string> &y_available)
{
	y_available = list_of_y_available;
}

void OpenSMOKE_PostProcessor_ICEM_MultiZone::ImportSelectedAxis(int x_axis, vector<int> y_axis, BzzMatrix &xAxis, BzzMatrix &yAxis, std::string &name_x, std::string &name_y, vector<string> &names_lines)
{
